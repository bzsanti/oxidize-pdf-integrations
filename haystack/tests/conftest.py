...
def _write_second_pdf(target: pathlib.Path) -> pathlib.Path:
    """A different PDF for multi-source batching tests — distinct text so
    cross-source leakage is detectable.
    """
    doc = op.Document()
    page = op.Page.a4()
    page.set_font(op.Font.HELVETICA, 14.0)
    page.text_at(50.0, 760.0, "Appendix: References")
    page.set_font(op.Font.HELVETICA, 12.0)
    page.text_at(50.0, 730.0, "Cross-source marker APX-SOURCE-TWO inside.")
    doc.add_page(page)
    doc.save(str(target))
    return target
...
@pytest.fixture
def second_pdf(tmp_path: pathlib.Path) -> pathlib.Path:
    return _write_second_pdf(tmp_path / "appendix.pdf")
