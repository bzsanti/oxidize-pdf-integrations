"""Semantic regression tests for ``OxidizePdfLoader(mode='rag')``.

These tests verify the chunks emitted are **element-disjoint** — no
``Document``'s ``page_content`` is a substring of another's, and each
unique source marker appears in exactly one ``Document``.

Background: the sibling llama-index-readers-oxidize-pdf reader 0.1.0
was published with shape-only tests that verified metadata fields
existed but never inspected chunk content for accumulation. The
underlying bridge bug (oxidize-pdf-core 2.5.4 ``HybridChunker`` overlap
branch re-injecting flushed elements) made every chunk i+1 contain
chunk i as a prefix, rendering both readers unusable for RAG ingestion
despite the metadata shape looking healthy.

This file exists so the same gap cannot recur silently in the LangChain
loader. Failure here = either the bridge regressed or the loader
stopped propagating the chunker's output verbatim. Either is a release
blocker.
"""
...
def multi_section_pdf(tmp_path: pathlib.Path) -> pathlib.Path:
    ...
    Exercises the longer element stream where the 0.1.0 LlamaIndex bug
    manifested most dramatically (each later chunk accumulated every
    prior chunk).
    """
    doc = op.Document()
...
# ── Generic semantic assertions (mirror LlamaIndex reader) ───────────────


def _assert_documents_pairwise_disjoint(docs) -> None:
    for i in range(len(docs)):
        for j in range(i + 1, len(docs)):
            ti, tj = docs[i].page_content, docs[j].page_content
            assert ti, f"docs[{i}].page_content is empty"
            assert tj, f"docs[{j}].page_content is empty"
            assert ti not in tj, (
                f"docs[{i}].page_content is a substring of docs[{j}].page_content — "
                f"quadratic accumulation bug regressed.\n"
                f"  i={ti!r}\n  j={tj!r}"
            )
            assert tj not in ti, (
                f"docs[{j}].page_content is a substring of docs[{i}].page_content — "
                f"quadratic accumulation bug regressed.\n"
                f"  i={ti!r}\n  j={tj!r}"
            )
...
def _assert_marker_appears_exactly_once(docs, marker: str) -> None:
    occurrences = sum(1 for d in docs if marker in d.page_content)
    assert occurrences == 1, (
        f"marker {marker!r} must appear in exactly one Document; "
        f"found in {occurrences}.\n"
        f"  texts: {[d.page_content for d in docs]}"
    )
...
class TestRagModeChunkDisjointness:
    ...
    def test_title_plus_paragraphs_documents_are_disjoint(
        ...
        docs = OxidizePdfLoader(disjointness_pdf, mode="rag").load()
    ...
    def test_multi_section_documents_are_disjoint(
        ...
        docs = OxidizePdfLoader(multi_section_pdf, mode="rag").load()
